...
const addContact = async (req, res, next) => {
  try {
    ...
    const newContact = await collection.find(contact).toArray();
    res.status(201).send({_id: newContact[0]._id});   
  } catch (err) {
    next(err);
  }
}
...
module.exports = { getAllContacts, getContactById, deleteContactById, updateContactById, addContact };